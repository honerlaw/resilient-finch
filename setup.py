...
def _get_uint_prop(
    device_id: int, selector: int, scope: int = kAudioObjectPropertyScopeGlobal
) -> int:
...
def _find_builtin_output() -> tuple[int, str] | None:
    for dev_id in _get_devices():
        if (
            _get_uint_prop(dev_id, kAudioDevicePropertyTransportType)
...
def _set_default_output(device_id: int) -> None:
    ...
    status = _CA.AudioObjectSetPropertyData(
        kAudioObjectSystemObject, ctypes.byref(addr), 0, None, ctypes.sizeof(val), ctypes.byref(val)
    )
    if status != 0:
...
def setup() -> None:
    ...
    if not _set_sample_rate(capture_id, 16000.0):
        print(
            "         Warning: could not set sample rate — set it manually to 16000 Hz in Audio MIDI Setup"
        )
    else:
        print("         done")
    ...
    print("  Wrote  ~/.resilient-finch/config.json")
